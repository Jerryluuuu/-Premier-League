...
ggplot(team_stats_away, aes(x = Avg_AC, y = Away_Win_Rate)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
### The relationshp between win rates and teams shots on target 
...
Teams are shown as color-coded dots in both plots, with team names and colours corresponded by the legend on the right. Although it's evident from both graphs that shooting accuracy and winning are correlated, it's crucial to remember that correlation does not imply causation and that other factors might also play a role in a team's performance. When analysing the variables influencing a team's performance, analysts may find these visualisations to be helpful resources.
